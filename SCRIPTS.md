...
| [game/tic-tac-toe.lua](game/tic-tac-toe.lua) | Endless Tic-Tac-Toe versus a depth-limited alpha-beta minimax AI. Sliding-piece variant: each side keeps at most 3 marks, so placing a 4th removes that side's oldest (shown dimmed) — the board never fills and there are no draws, play runs until someone wins. UP/DOWN cycles empty cells (skips occupied), OK places X; on touch-capable boards you can also tap a cell to place there directly. Player is X and always moves first. | `/unigeek/games/tic-tac-toe.txt` (wins/losses as JSON) |
